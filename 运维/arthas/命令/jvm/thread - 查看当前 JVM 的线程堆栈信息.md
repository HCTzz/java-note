<center><font size="4"><b>查看当前 JVM 的线程堆栈信息</b></font></center>
...
### [#](https://arthas.aliyun.com/doc/thread.html#参数说明)参数说明
...
### [#](https://arthas.aliyun.com/doc/thread.html#cpu-使用率是如何统计出来的)cpu 使用率是如何统计出来的？

这里的 cpu 使用率与 linux 命令`top -H -p <pid>` 的线程`%CPU`类似，采样一段间隔时间内，当前 JVM 里各个线程的增量 cpu 时间与采样间隔时间的比例。

#### [#](https://arthas.aliyun.com/doc/thread.html#工作原理说明)工作原理说明：
...
### [#](https://arthas.aliyun.com/doc/thread.html#使用参考)使用参考

#### [#](https://arthas.aliyun.com/doc/thread.html#支持一键展示当前最忙的前-n-个线程并打印堆栈)支持一键展示当前最忙的前 N 个线程并打印堆栈：
...
#### [#](https://arthas.aliyun.com/doc/thread.html#当没有参数时-显示第一页线程的信息)当没有参数时，显示第一页线程的信息
...
#### [#](https://arthas.aliyun.com/doc/thread.html#thread-all-显示所有匹配的线程)thread --all, 显示所有匹配的线程

显示所有匹配线程信息，有时需要获取全部 JVM 的线程数据进行分析。

#### [#](https://arthas.aliyun.com/doc/thread.html#thread-id-显示指定线程的运行堆栈)thread id, 显示指定线程的运行堆栈
...
#### [#](https://arthas.aliyun.com/doc/thread.html#thread-b-找出当前阻塞其他线程的线程)thread -b, 找出当前阻塞其他线程的线程
...
#### [#](https://arthas.aliyun.com/doc/thread.html#thread-i-指定采样时间间隔)thread -i, 指定采样时间间隔
...
#### [#](https://arthas.aliyun.com/doc/thread.html#thread-state-查看指定状态的线程)thread --state ，查看指定状态的线程
...
14   arthas-timer                   system          9          WAITING   0.0       0.000      0:0.00
```